...
def get_consultations(patient_id: str, db: Session = Depends(get_db)):
    return doctor_service.get_patient_consultations(db, patient_id)

@router.post("/feedback", status_code=201)
def submit_doctor_feedback(data: DoctorFeedbackCreate, db: Session = Depends(get_db)):
    return doctor_service.add_doctor_feedback(db, data.model_dump())

@router.get("/recommendations/{patient_id}")
def get_recommendations_for_review(patient_id: str, db: Session = Depends(get_db)):
